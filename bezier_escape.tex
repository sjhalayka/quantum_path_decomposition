...
In this paper we will be quantizing the kinematic and gravitational time dilation by casting them to a smaller floating point number. 
The quantized step size (e.g. epsilon) in general is $\epsilon = 2^{-m}$ where $m$ is the number of mantissa bits in the floating point number.
In this paper, the bit count $n$ includes the number of mantissa bits $m$, plus one sign bit.
